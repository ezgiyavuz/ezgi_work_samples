...
movie = pd.read_csv("Homeworks/ezgi_yavuzay/Odev_Hafta_5/movie.csv")
movie.head()
movie.shape

# UserID, film adı, filme verilen oy ve zaman bilgisini içeren veri seti
rating = pd.read_csv("Homeworks/ezgi_yavuzay/Odev_Hafta_5/movie.csv")
rating.head()
rating.shape
...
#random_user = 108170
random_user = int(pd.Series(user_movie_df.index).sample(1, random_state=46).values)
random_user
...
movie = pd.read_csv("Homeworks/ezgi_yavuzay/Odev_Hafta_5/movie.csv")
rating = pd.read_csv("Homeworks/ezgi_yavuzay/Odev_Hafta_5/rating.csv")
